...
public class CLOSE_RED_9_BALL extends OpMode {
        ...
        switch (pathState) {
            ...
            case shoot_preload:
                if (!follower.isBusy()) {
                    ...
                    // Indexing Logic: Stop transfer if ball detected while waiting for RPM
                    if (!shooterSub.isReady()) {
                        intake.setPower(1.0);
                        transfer.setPower(ballInTransfer ? 0.0 : 0.6);
                    } else {
                        // Firing Logic: Force push
            ...
            case drive_pickUp1EndPOS_shootPOS:
                // Maintain index while driving back
                intake.setPower(1.0);
                transfer.setPower(ballInTransfer ? 0.0 : 0.6);
    ...
    public void loop() {
        follower.update();
        ballInTransfer = stopSensor.getState(); // Read sensor (True = Ball Detected)
        shooterSub.update(0, false); // Heartbeat
        statePathUpdate();
